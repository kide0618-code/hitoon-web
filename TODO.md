* ストアのカード (artists/\[id]/client.tsx): serialNumber={card.currentSupply + 1} と totalSupply
  を渡すように変更。例えば5枚販売済みなら #006 / 100 と表示されます。

  * 詳細ダイアログのカード (card-detail-dialog.tsx): 同様にシリアルナンバーを表示。

  カード面の右下にレアリティバッジと並んで #006 / 100 の形式で表示されます（card\_\_meta
  領域）。コレクションのカードは購入時に確定したシリアルナンバーがそのまま表示される既存の動作を維持しています。
*
*
* カードUIの変更点
