...
* 1アーティスト = 複数カード作成可能（アルバム別、シングル別など）
* 各カードに画像・楽曲名・レアリティ・フレームを個別設定
* カードコンテンツ（画像・楽曲名）= DB管理、フレーム（枠・エフェクト）= TypeScript Config管理
...
#### 2.2.3 Card Management (カード管理)

| ID      | Feature  | Priority | Description   |
| ------- | -------- | -------- | ------------- |
| CRD-001 | カード作成    | P0       | 画像アップロード、情報入力 |
| CRD-002 | レアリティ設定  | P0       | 価格・在庫・フレーム設定 |
| CRD-003 | プレビュー    | P1       | 作成前のカード表示確認   |
| CRD-004 | 公開/非公開切替 | P0       | カードの販売状態管理    |
| CRD-005 | 限定コンテンツ管理 | P0       | カード別のボーナスコンテンツ設定 |
...
Required:
  - name: String (カード名)
  - artist_id: UUID (アーティスト)
  - card_image_url: String (画像URL, 最大10MB, 600x800px推奨)
  - rarity: 'NORMAL' | 'RARE' | 'SUPER_RARE'
  - price: Number (JPY)
  - frame_template_id: String (config/frame-templates.ts のID)
...
Per Card (exclusive_contents):
  - bonus_contents: Array<{
      url: String (外部URL: YouTube, Vimeo, SoundCloud等)
...
* 各カードは個別に作成（画像・楽曲情報・レアリティ・フレームを直接指定）
* 1アーティストに対して複数カード作成可能
* フレーム（枠・エフェクト）は `config/frame-templates.ts` で管理（エンジニア対応）
...
※ card_visuals テーブルは廃止。画像・楽曲情報は cards テーブルに統合
※ FrameTemplate (フレーム・エフェクト) は config/frame-templates.ts で管理
```
...
| description         | TEXT        |                                          | 説明                   |
| card\_image\_url    | TEXT        | NOT NULL                                 | アーティスト画像URL        |
| song\_title         | TEXT        |                                          | 楽曲名                 |
| subtitle            | TEXT        |                                          | サブタイトル              |
| frame\_template\_id | TEXT        | NOT NULL, DEFAULT 'classic-normal'       | フレームテンプレートID       |
...
**Note**: card\_visuals テーブルは廃止済み。画像・楽曲情報は cards テーブルに統合。
...
| GET    | /api/purchases/:id            | 購入詳細               | Required + Ownership |
| GET    | /api/cart                     | カート取得              | Required             |
| POST   | /api/cart                     | カート操作              | Required             |
| GET    | /api/me                       | 現在のユーザー情報          | Required             |
| GET    | /api/og/card                  | OG画像生成             | -                    |
...
| Method | Endpoint                                    | Description   | Auth  |
| ------ | ------------------------------------------- | ------------- | ----- |
| GET    | /api/admin/artists                          | アーティスト一覧     | Admin |
| POST   | /api/admin/artists                          | アーティスト作成     | Admin |
| PUT    | /api/admin/artists/:id                      | アーティスト更新     | Admin |
| PUT    | /api/admin/artists/:id/payouts              | 出金ステータス更新   | Admin |
| GET    | /api/admin/cards                            | カード一覧         | Admin |
| POST   | /api/admin/cards                            | カード作成         | Admin |
| PUT    | /api/admin/cards/:id                        | カード更新         | Admin |
| POST   | /api/admin/cards/:id/contents               | 限定コンテンツ作成   | Admin |
| PUT    | /api/admin/cards/:id/contents/:contentId    | 限定コンテンツ更新   | Admin |
| POST   | /api/admin/upload                           | 画像アップロード    | Admin |
...
| Cart          | `/cart`           | カート（購入前の確認）      |
| Activity      | `/activity`       | 購入完了・アクティビティ     |
...
| Login            | `/login`           | ログイン           |
| Signup           | `/signup`          | 新規登録           |
| Forgot Password  | `/forgot-password` | パスワード再設定リクエスト |
| Reset Password   | `/reset-password`  | パスワード再設定       |
...
| ------------- | ---------------- | ----------- |
| Terms         | `/terms`         | 利用規約        |
| Terms of Sale | `/terms-of-sale` | 特定商取引法に基づく表記 |
...
| Artist Detail    | `/admin/artists/:id`       | アーティスト詳細・編集  |
| Cards            | `/admin/cards`             | カード管理          |
| Card Detail      | `/admin/cards/:id`         | カード詳細・編集      |
| Card Content     | `/admin/cards/:id/contents/:contentId` | 限定コンテンツ管理 |
| Purchases        | `/admin/purchases`         | 購入履歴           |

### 7.2 Card Visual & Frame System

**Reference Design**: `/docs/card-image.png`

カードは「コンテンツ（DB: cards）」と「フレーム（装飾）」の2層構造:

* **Card** (DB: cards): アーティスト画像、楽曲名、レアリティ等 - 管理画面から登録
* **FrameTemplate** (TypeScript): フレーム、エフェクト - `config/frame-templates.ts` で管理

```
┌─────────────────────────────┐
│  ┌───────────────────────┐  │
│  │                       │  │
│  │    Artist Visual      │  │  ← Layer 3: Card (DB: card_image_url)
│  │                       │  │
│  ├───────────────────────┤  │  ← Layer 2: FrameTemplate (Config)
│  │ IDOL: {artist_name}   │  │  ← Layer 4: Card (DB: song_title, subtitle)
│  │ SONG: {song_title}    │  │
│  └───────────────────────┘  │
│  Rarity: {N/R/SR}  #{serial}│  ← Layer 5: Metadata (自動生成)
│  Owned: {count}             │
│  ┌─────┐                    │
│  │ QR  │  BONUS CONTENT     │  ← Layer 6: Bonus Link
│  └─────┘                    │
└─────────────────────────────┘  ← Layer 1: FrameTemplate (Config)
```
...
| Term                  | Definition                           |
| --------------------- | ------------------------------------ |
| **Card**              | アーティストのデジタルトレーディングカード（画像・楽曲情報を含む）  |
| **FrameTemplate**     | カードのフレーム・エフェクト - TypeScript Config管理 |
...
| 1.3.0   | 2026-02-03 | -      | 用語整理: card\_templates → card\_visuals、FrameTemplate概念追加（フレーム・エフェクトはTypeScript Config管理） |
| 1.4.0   | 2026-03-12 | -      | card\_visuals廃止・cardsテーブルに統合、画面一覧・API・管理画面を実装状況に合わせて更新 |
