...
- 1ビジュアル → 3カード (NORMAL, RARE, SUPER_RARE)
- ビジュアル（画像・楽曲名）= DB管理、フレーム（枠・エフェクト）= TypeScript Config管理
...
| ADM-001 | ロール管理 | P2       | admin, super_admin（将来）  |
...
#### 2.2.3 Card Visual Management (カードビジュアル管理)

| ID      | Feature         | Priority | Description                |
| ------- | --------------- | -------- | -------------------------- |
| VIS-001 | ビジュアル作成  | P0       | 画像アップロード、情報入力 |
| VIS-002 | レアリティ設定  | P0       | 3パターンの価格・在庫設定  |
| VIS-003 | プレビュー      | P1       | 作成前のカード表示確認     |
| VIS-004 | 公開/非公開切替 | P0       | カードの販売状態管理       |

**Visual Input Fields**:

```
Required:
  - visual_name: String (管理用識別子: "1st Album", "Summer Single")
...
- 1アーティストに対して複数ビジュアル作成可能
- 例: アルバムA用ビジュアル、シングルB用ビジュアル
- `visual_name`で管理画面上での識別を容易に
...
```
                                               ...
                                               └──────┬──────┘
                                                     ...
                                                     N:1
                                                      │
┌─────────────┐       ┌──────────────────┐       ┌─────────────┐
│   artists   │──1:N──│   card_visuals   │──1:3──│    cards    │
│ (is_featured)│       └──────────────────┘       └─────────────┘
└─────────────┘                                        │
                                                       │
                              ┌─────────────────────────┤
...
| ---------- | ----------- | --------------------------------- | ------------------ |
| id         | UUID        | PK                                |                    |
| user_id    | UUID        | FK → auth.users, UNIQUE, NOT NULL | 管理者のユーザーID |
...
| member_count  | INT         | DEFAULT 0                     | ファン数（集計値）  |
...
| name             | TEXT        | NOT NULL      | ビジュアル名（管理用識別子）         |
| artist_image_url | TEXT        | NOT NULL      | カード用アーティスト画像（最大10MB） |
| song_title       | TEXT        |               | 楽曲名                               |
...
| -------------- | ----------- | --------------------------------------- | ----------- |
| id             | UUID        | PK                                      |             |
| visual_id      | UUID        | FK → card_visuals                       |             |
| artist_id      | UUID        | FK → artists                            |             |
| name           | TEXT        | NOT NULL                                | カード名    |
| description    | TEXT        |                                         | 説明        |
| rarity         | TEXT        | CHECK IN ('NORMAL','RARE','SUPER_RARE') |             |
| price          | INT         | NOT NULL                                | 価格（円）  |
| total_supply   | INT         | NULL = unlimited                        | 発行上限    |
| current_supply | INT         | DEFAULT 0                               | 現在発行数  |
| is_active      | BOOLEAN     | DEFAULT TRUE                            |             |
| created_at     | TIMESTAMPTZ |                                         |             |
| updated_at     | TIMESTAMPTZ |                                         |             |

**Unique Constraint**: `(visual_id, rarity)`

#### exclusive_contents (限定コンテンツ)

| Column        | Type        | Constraints                        | Description          |
| ------------- | ----------- | ---------------------------------- | -------------------- |
| id            | UUID        | PK                                 |                      |
| card_id       | UUID        | FK → cards                         | レアリティ別に紐付け |
...
**Note**: card_id単位で紐付け → SR専用コンテンツなどを設定可能
...
| serial_number              | INT         | NOT NULL                                    | シリアルNo（レアリティ別採番） |
| price_paid                 | INT         | NOT NULL                                    | 購入時価格                     |
| quantity_in_order          | INT         | DEFAULT 1                                   | 注文内の購入枚数               |
| stripe_checkout_session_id | TEXT        | NOT NULL                                    | 冪等性チェック用               |
...
- 例: NORMAL #42/∞, RARE #15/100, SUPER_RARE #3/30
...
| GET    | /api/artists/:id/visuals      | アーティストのビジュアル一覧 | -                    |
| GET    | /api/cards                    | カード一覧                   | -                    |
...
| POST   | /api/admin/visuals     | ビジュアル作成     | Admin |
| PUT    | /api/admin/visuals/:id | ビジュアル更新     | Admin |
...
| Visuals   | `/admin/visuals` | カードビジュアル管理 |
...
- **CardVisual** (DB): アーティスト画像、楽曲名 - 管理画面から登録
...
│  ┌───────────────────────┐  │
│  │                       │  │
│  │    Artist Visual      │  │  ← Layer 3: CardVisual (DB)
│  │                       │  │
│  ├───────────────────────┤  │  ← Layer 2: FrameTemplate (Config)
│  │ IDOL: {artist_name}   │  │  ← Layer 4: CardVisual (DB)
│  │ SONG: {song_title}    │  │
│  └───────────────────────┘  │
...
| Webhook重複実行              | High   | Medium      | checkout_session_idでユニーク制約      |
...
| **CardVisual**        | カードのコンテンツ（画像、楽曲名）- DB管理           |
| **FrameTemplate**     | カードのフレーム・エフェクト - TypeScript Config管理 |
| **Rarity**            | カードの希少度（NORMAL, RARE, SUPER_RARE）           |
...
| 1.3.0   | 2026-02-03 | -      | 用語整理: card_templates → card_visuals、FrameTemplate概念追加（フレーム・エフェクトはTypeScript Config管理） |
